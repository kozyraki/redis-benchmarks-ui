...
RedisBenchmark.prototype.run = function(num_requests, callback) {
  ...
  console.log("%s %s", this.getBenchmarkCommand(), args);
  ...
  child.stderr.on('output', function(output) {
    // Log errors
    console.log("[ERR]: %s", output);
  });
  ...
  child.on('exit', function(exitCode) {
    ...
    // Parse the output of redis-benchmark to an object.
    lines = output.split("\n");
    for (var i in lines) {
